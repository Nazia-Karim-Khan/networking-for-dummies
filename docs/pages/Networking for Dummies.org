...
#+END_SRC
After running these commands inside conainers' shell, it will look something like this.
...
In the containers' interactive shell:
#+BEGIN_SRC shell
sudo ip link set dev veth-meena up
sudo ip link set dev veth-raju up
#+END_SRC
_Note: While running these commands in the containers' interactive shell, some of you may face authentication related errors. In that case try running these commands without #+BEGIN_SRC shell sudo #+END_SRC._
...
#+BEGIN_SRC shell
sudo ip addr add 10.2.3.1/24 dev nfd-rtr
#+END_SRC
...
In containers' interactive shell:
#+BEGIN_SRC shell
